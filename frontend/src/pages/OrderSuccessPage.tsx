      ...
      <div className="bg-white rounded-3xl border border-gray-100 p-8 text-center shadow-sm space-y-4">
        ...
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-brand-700 bg-brand-50 px-3 py-1 rounded-full">
            Direct Farm Order Confirmed
          </span>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 mt-2">
            Thank You for Supporting Direct Farmers!
          </h1>
          <p className="text-xs sm:text-sm text-gray-500 mt-1">
            Order Number: <span className="font-mono font-bold text-gray-900">#{order?.orderNumber || id}</span>
          </p>
        </div>

        <p className="text-xs text-gray-600 max-w-md mx-auto leading-relaxed">
          The farmer has been notified and will harvest your produce fresh. You can track the status
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
          <Link
            ...
            className="w-full sm:w-auto bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs px-6 py-3 rounded-full shadow-md shadow-brand-600/20 transition flex items-center justify-center gap-2"
          >
            <Truck className="w-4 h-4" /> Track Harvest & Dispatch
          </Link>
          <Link
            to="/marketplace"
      ...
      {order && (
        <div className="bg-white rounded-3xl border border-gray-100 p-6 sm:p-8 shadow-sm space-y-6">
              ...
              <div key={item.id} className="py-3 flex items-center justify-between text-xs">
                <div>
                  ...
                  <p className="text-[11px] text-gray-500">
                    Qty: {item.quantity} × ₹{item.unitPrice} • Farmer: {item.farmer?.name || 'Local Farm'}
                  </p>
                </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-gray-100 text-xs">
            <div>
              <span className="text-gray-400 font-medium block mb-1">Delivery Destination:</span>
              <p className="font-bold text-gray-900">{order.shippingAddress}</p>
              <p className="text-gray-600 mt-0.5">Phone: {order.contactPhone}</p>
            </div>
            <div>
              <span className="text-gray-400 font-medium block mb-1">Payment Method:</span>
              <p className="font-bold text-gray-900">{order.paymentMethod}</p>
              <p className="text-emerald-700 font-semibold mt-0.5">Status: {order.paymentStatus}</p>
            </div>
          </div>
        </div>
      )}
